...
class AutoParserTest {
    ...
    fun testParseQqQrc() {
        ...
        val result = AutoParser().parse(qrc)

        assertEquals(2, result.lines.size)
        val firstLine = result.lines[0] as KaraokeLine.MainKaraokeLine
        assertEquals("Qyc lyric", firstLine.syllables.joinToString("") { it.content })
        assertTrue(firstLine.syllables.none { it.content.contains("(0,400)") })
    }
